...
def set_angle(angle):
    ...
    return int(500 + (angle/50) * 2000)

def map_range(value, in_min, in_max, out_min, out_max):
    return int((value - in_max) * (out_max - out_min) / (in_max - in_min) + out_min)
...
while True:
    ...
    print(f"Digital: {data.value()} , Analog: {adc_value} , Sevro: {servo.duty_ns()}")
    sleep(0.1)
